...
// Compara promedios institucionales, por grupo y por materia entre P1, P2 y P3
    ...
    const [institucional, porGrupo, porMateria, periodos] = await Promise.all([
      ...
      db.query(`
        SELECT
          ...
          COUNT(DISTINCT ra.actividad_id)    AS actividades
        FROM resultados_actividades ra
        JOIN actividades a  ON a.id  = ra.actividad_id AND a.activa = TRUE
        JOIN grupos      g  ON g.id  = a.grupo_id
        WHERE g.colegio_id = ?
...
module.exports = { reporteDocente, reporteGrupoMateria, resumenColegio, alertasDocente, alertasColegio, metricasColegio, comparativasPeriodos, gemeloDigital };

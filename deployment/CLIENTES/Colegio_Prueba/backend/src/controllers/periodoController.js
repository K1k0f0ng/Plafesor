...
async function listar(req, res) {
  ...
  try {
    const [filas] = await db.query(`
      SELECT id, colegio_id, nombre, numero, ano_lectivo,
...
async function crear(req, res) {
  const { nombre, numero, ano_lectivo, fecha_inicio, fecha_fin } = req.body;
  const colegio_id = req.usuario.colegio_id;

  if (!colegio_id || !nombre || !numero || !ano_lectivo || !fecha_inicio || !fecha_fin) {
    return res.status(400).json({ error: 'Todos los campos son obligatorios' });
  }
  if (!['1', '2', '3'].includes(String(numero))) {
    return res.status(400).json({ error: 'El número de período debe ser 1, 2 o 3' });
  }
  ...
  try {
    const [result] = await db.query(`
      INSERT INTO periodos_academicos (colegio_id, nombre, numero, ano_lectivo, fecha_inicio, fecha_fin)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [colegio_id, nombre, String(numero), ano_lectivo, fecha_inicio, fecha_fin]);
...
async function actualizar(req, res) {
  ...
  try {
    ...
    `, [nombre || null, numero ? String(numero) : null, ano_lectivo || null, fecha_inicio || null, fecha_fin || null, id, colegio_id]);

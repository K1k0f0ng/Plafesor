...
async function libroCalificaciones(req, res) {
  ...
  try {
    // 1. Actividades del grupo+materia+periodo con promedio por actividad
    ...
    const [actividades] = await db.query(`
      SELECT a.id, a.titulo, a.creado_en,
        ...
        ROUND(AVG(mejores.mejor_nota), 1) AS promedio_actividad
      FROM actividades a
      LEFT JOIN (
    ...
    filas.forEach(f => {
      ...
      if (f.actividad_id !== null && f.nota !== null) {
        estudiantesMap[f.estudiante_id].notas[f.actividad_id] = f.nota;
      }
    });
    ...
    res.json({ data: { actividades, estudiantes } });
  } catch (err) {

...
async function generarMensajeWhatsApp(datos, nombreColegio) {
  ...
  try {
    ...
    const resp = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Eres el asistente de Playfesor para colegios colombianos. Con estos datos, escribe un análisis ejecutivo en máximo 3 oraciones (máx 100 palabras) para el director/rector. Usa escala MEN colombiana. Tono profesional y directo. Destaca lo más urgente:\n\n${contexto}`,
      }],
    });
  ...
  const lineas = [
    ...
    ``,
    `🤖 *Análisis Playfesor IA:*`,
    narrativa,
    ``,
    `_Ver detalle completo en playfesor.co_`,

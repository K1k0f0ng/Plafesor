...
const P_LABEL  = { '1': 'Período 1', '2': 'Período 2', '3': 'Período 3' };
const P_COLOR  = { '1': 'var(--color-primario)', '2': '#f093fb', '3': '#43e97b' };
const P_BG     = { '1': '#f0f2ff', '2': '#fdf0ff', '3': '#f0fff6' };
...
function BarraComparativa({ datos }) {
  const max = 5;
  return (
...
function TablaComparativa({ filas, campoId, campoNombre, etiquetaNombre }) {
      ...
      <table style={es.tabla}>
        ...
        <tbody>
          {ids.map(id => {
            ...
            const p1        = fila[0]?.promedio ? parseFloat(fila[0].promedio) : null;
            const p2        = fila[1]?.promedio ? parseFloat(fila[1].promedio) : null;
            const p3        = fila[2]?.promedio ? parseFloat(fila[2].promedio) : null;
            const tendencia = flecha(p1, p3 ?? p2);
            ...
            return (
              <tr key={id} style={es.tr}>
                ...
                {[p1, p2, p3].map((val, i) => {
                  const nivel = nivelMEN(val);
                  const arr   = i > 0 ? flecha([p1, p2, p3][i - 1], val) : null;
                  return (
...
export default function Comparativas() {
  ...
  // Calcular tendencia institucional general (P1 → último período con datos)
  const promedioP = p => {
    const item = inst.find(d => String(d.periodo) === p);
    return item ? parseFloat(item.promedio) : null;
  };
  const p1 = promedioP('1'), p2 = promedioP('2'), p3 = promedioP('3');
  const tendenciaGlobal = flecha(p1, p3 ?? p2);
...
const es = {
  ...
  tabActivo: { background: 'linear-gradient(135deg, var(--color-primario), var(--color-secundario))', borderColor: 'var(--color-primario)', color: '#fff' },

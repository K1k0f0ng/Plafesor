...
const NUMERO_LABELS = { '1': 'Período 1', '2': 'Período 2', '3': 'Período 3' };
...
export default function Periodos() {
  ...
  const [form, setForm] = useState({ nombre: '', numero: '1', ano_lectivo: '2026', fecha_inicio: '', fecha_fin: '' });
  ...
  function abrirCrear() {
    setEditando(null);
    setForm({ nombre: '', numero: '1', ano_lectivo: '2026', fecha_inicio: '', fecha_fin: '' });
    setError('');
    setMostrarForm(true);
  }
                ...
                <div key={p.id} style={{ ...es.card, borderTop: `4px solid ${sem.color}`, background: p.activo ? '#fafffe' : '#fff' }}>
                  <div style={es.cardCabecera}>
                    <div>
                      ...
                      <div style={es.cardSub}>{NUMERO_LABELS[p.numero]} · {p.ano_lectivo}</div>
                    </div>

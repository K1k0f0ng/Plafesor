...
export default function LibroNotas() {
  ...
  const sinEntregas     = libro ? libro.estudiantes.filter(e => e.promedio === null).length : 0;
              ...
              <div style={es.tablaWrapper}>
                <table style={es.tabla}>
                  ...
                  <tbody>
                    ...
                    <tr style={{ borderTop: '2px solid var(--color-primario)', background: '#f8f9ff' }}>
                      <td style={{ ...es.tdFijo, fontWeight: '800', color: 'var(--color-primario)', background: '#f0f0ff' }}>
                        Promedio actividad
                      </td>
                      {libro.actividades.map(a => (
                        <td key={a.id} style={es.tdCentro}>
...
const es = {
  ...
  btn:          { background: 'linear-gradient(135deg,var(--color-primario),var(--color-secundario))', color: '#fff', border: 'none', borderRadius: '8px', padding: '11px 22px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' },

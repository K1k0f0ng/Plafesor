  ...
  return (
    <div style={es.boletin}>
      ...
      <div style={es.encabezado}>
        <div style={es.logoCirculo}><img src="/logo-icon.png" alt={NOMBRE_INSTITUCION} style={{ width: '32px', height: '32px' }} /></div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ marginBottom: '18px' }}>
        ...
        <table style={es.tablaImpresa}>
          ...
          <tbody>
            ...
            <tr style={{ background: '#f8f9ff', borderTop: '2px solid var(--color-primario)' }}>
              <td style={{ ...es.tdImpreso, fontWeight: '800', color: '#1a1a2e' }}>PROMEDIO GENERAL</td>
              <td style={{ ...es.tdImpreso, textAlign: 'center', fontWeight: '800', fontSize: '17px', color: 'var(--color-primario)' }}>
                {b.promedio_general ?? '—'}
              </td>
              <td style={{ ...es.tdImpreso, textAlign: 'center' }}>
        ...
        <div style={es.asistenciaGrid}>
          {[
            ...
            { num: b.asistencia.tasa_asistencia != null ? `${b.asistencia.tasa_asistencia}%` : '—',
              label: 'Tasa', color: 'var(--color-primario)', destacado: true },
          ].map(({ num, label, color, destacado }) => (
            <div key={label} style={{ ...es.asistenciaCard, ...(destacado ? { background: '#f3f4ff', border: '2px solid var(--color-primario)' } : {}) }}>
      ...
      {/* Pie */}
      <div style={es.pie}>
        {b.colegio.nombre} · Documento oficial · {formatFecha()}
      </div>
    </div>
  );
}
...
const es = {
  ...
  encabezado:      { display: 'flex', alignItems: 'center', gap: '14px', borderBottom: '3px solid var(--color-primario)', paddingBottom: '18px', marginBottom: '18px' },
  logoCirculo:     { width: '48px', height: '48px', borderRadius: '10px', background: 'linear-gradient(135deg,var(--color-primario),var(--color-secundario))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', flexShrink: 0 },
  nombreColegio:   { fontSize: '17px', fontWeight: '800', color: '#1a1a2e' },
  subtitleColegio: { fontSize: '12px', color: '#888', marginTop: '2px' },
  periodoChip:     { background: 'linear-gradient(135deg,var(--color-primario),var(--color-secundario))', color: '#fff', borderRadius: '20px', padding: '6px 16px', fontWeight: '800', fontSize: '13px', flexShrink: 0 },
  ...
  seccionTitulo:   { fontSize: '10px', fontWeight: '800', color: 'var(--color-primario)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' },
  ...
  btnGenerarObs:{ background: 'linear-gradient(135deg,var(--color-secundario),var(--color-primario))', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px 18px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', marginBottom: '12px' },

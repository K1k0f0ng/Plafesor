...
const COLORES_MATERIA = ['var(--color-primario)', 'var(--color-secundario)', '#f093fb', '#4facfe', '#43e97b', '#fa709a'];
          ...
          <div style={es.planBanner}>
            ...
            {planes.map(plan => (
              <div key={plan.id} style={es.planCard}>
                ...
                <div style={es.planDiag}><IconBarChart size={13} style={{ marginRight: 5, verticalAlign: 'middle', color: 'var(--color-primario)' }} />{plan.diagnostico}</div>
...
const es = {
  ...
  planBtnVer: { background: 'none', border: 'none', color: 'var(--color-primario)', cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', padding: 0 },
  planDiag: { fontSize: 13, color: '#555', background: '#f9f9ff', border: '1px solid #e8e8ff', borderRadius: 6, padding: '6px 10px' },
  planTexto: { marginTop: 10, background: '#f8f9fa', borderRadius: 8, padding: 14, borderLeft: '4px solid var(--color-primario)' },

...
const SUGERENCIAS = [
  '¿Cómo está el promedio de mis grupos?',
  '¿Qué estudiantes necesitan más atención?',
  '¿Cuál es mi grupo con más riesgo académico?',
  '¿Cómo está la asistencia este mes?',
  '¿Qué actividad tuvo el peor resultado?',
  '¿Qué recomendaciones tienes para mejorar mis notas?',
];
...
export default function CoPilotoDocente() {
  ...
  const [mensajes, setMensajes] = useState([{
    rol: 'asistente',
    texto: `¡Hola, ${usuario.nombre}! Soy tu Copiloto Académico personal.\n\nTengo acceso en tiempo real a los datos de tus grupos, el rendimiento de tus estudiantes, la asistencia y las alertas académicas.\n\nPuedes preguntarme sobre cualquier aspecto de tus clases y te daré recomendaciones concretas. ¿En qué te ayudo hoy?`,
  }]);
  ...
  const enviar = useCallback(async (texto) => {
    ...
    setCargando(true);
    try {
      const resp = await axiosAuth.post(`/api/copiloto/docente/${usuario.id}`, { pregunta: q });
  ...
  return (
    <div style={es.pagina}>
      <Navbar titulo="Copiloto del Docente" />
      <div style={es.layout}>

        {/* Sidebar */}
        <div style={es.sidebar}>
          <button onClick={() => navigate('/dashboard-docente')} style={es.btnVolver}>← Volver</button>
          ...
          <div style={es.infoBadge}>
            <IconZap size={20} style={{ color: 'var(--color-primario)' }} />
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: '11px', color: '#999', lineHeight: '1.4' }}>
                El copiloto analiza tus grupos antes de cada respuesta
              </p>
            </div>
          </div>
        </div>

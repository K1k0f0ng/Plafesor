...
function dibujarPiar(doc, p) {
  ...
  doc.fillColor('#BBBBBB').fontSize(7.5).font('Helvetica')
     .text(`${p.colegio} · Documento oficial · ${fechaHoy()} — Borrador sujeto a validación conforme al Decreto 1421 de 2017`,
...
function generarPiarPDF(data) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: { Title: `PIAR - ${data.estudiante}`, Author: data.colegio || process.env.INSTITUCION_NOMBRE || 'Institución' },
      autoFirstPage: true,
    });

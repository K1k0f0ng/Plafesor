...
async function recibirWebhook(req, res) {
  ...
  try {
    ...
    if (!padres.length) {
      await enviarMensaje(telefonoIncoming, `Hola 👋 No encontramos una cuenta de padre/madre asociada a este número en ${process.env.INSTITUCION_NOMBRE || 'la institución'}. Comunícate con el colegio para activar tu acceso.`);
      return;
    }
    ...
    const nombreColegio = hijos[0]?.colegio || process.env.INSTITUCION_NOMBRE || 'la institución';
    const prompt = `Eres el asistente de ${nombreColegio}, un sistema de gestión académica colombiano. Responde de forma amigable, breve y clara al mensaje del padre/madre. Máximo 170 palabras.
...
Responde en español, tono cálido y profesional. Sin formato markdown (sin **, #, listas con -). Solo texto plano apropiado para WhatsApp.`;
    ...
    try {
      ...
      respuesta = `Hola ${padre.nombre}, en este momento no puedo procesar tu consulta. Por favor intenta de nuevo en unos minutos o ingresa a la plataforma de ${nombreColegio} para ver el detalle de tu hijo/a.`;
    }

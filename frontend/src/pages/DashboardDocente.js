...
import { ordenarPorApellido, formatearApellidoPrimero } from '../utils/ordenNombre';
import {
...
export default function DashboardDocente() {
  ...
  function exportarExcel() {
    ...
    const filas = ordenarPorApellido(Object.values(estudiantesMap), 'nombre').map(est => {
      const fila = { Estudiante: formatearApellidoPrimero(est.nombre) };
      const notasValidas = [];
              ...
              <div style={es.alertaLista}>
                {alertas.map((a, i) => (
                  <div key={i} style={es.alertaFila}>
                    <span style={es.alertaNombre}>{formatearApellidoPrimero(a.nombre_estudiante)}</span>
                    ...
                    <div style={es.listaNombres}>
                      {estudiantesUnicos.map(est => (
                        <button key={est.id} onClick={() => verFichaEstudiante(est.id)} style={es.filaNombre}>
                          <span>{formatearApellidoPrimero(est.nombre)}</span>
